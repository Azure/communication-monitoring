...
const subHtml: { [key: string]: HtmlBlockItems } = {}
...
function generateHtml(key: string, id: string) {
  ...
  let html = ''
  html +=
    `<dt class='sectionHeader'>${item.sectionName}</dt>` +
...
export function updateMediaStatsTable(mediaStatsData: MediaStatsData) {
  try {
    if (mediaStatsData) {
      ...
      if (
        allIds.filter((x) => !newIds.includes(x)).length ||
      ...
      for (const k of metricKeys) {
        ...
        if (dataValue !== undefined) {
          ...
          ;(document.getElementById(elementValueId) as HTMLElement).innerText =
            ''
          ;(document.getElementById(elementValueId) as HTMLElement).hidden =
            true
